...
**Wave 2 *(blocked on Wave 1 completion)*:** 28-02 and 28-03 can run in parallel after the target resolver and task metadata foundation exists.

**Wave 3 *(blocked on Waves 1 and 2 completion)*:** 28-04 verifies the integrated flow, documents behavior, and updates GSD completion artifacts.

**Cross-cutting constraints:**
- Existing v0.1.0a2 maintenance commands, `sorted/graph-*` artifact paths, JSON envelopes, and `wikify.graph-agent-tasks.v1` fields remain compatible.
- Maintenance uses an ephemeral target resolver over existing object/source/wiki/view/agent/control artifacts; it must not create a second knowledge store.
- Derived human views and agent exports are refreshed through explicit `wikify views` and `wikify agent export` tasks rather than silently patched or regenerated by `wikify maintain`.
- Generated wiki page repairs must preserve `source_refs` and `review_status` deterministically, independent of external verifier quality.
- Maintenance must not hide provider calls, embeddings, raw source rereads, sync/wikiize/view/export execution, or external agent execution behind graph maintenance.
...
| v0.2.0 Personal Wiki Core & Views | 22-28 | 6/7 | In Progress | — |
