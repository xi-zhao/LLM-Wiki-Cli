...
- `generated_page_preservation_failed`
- `patch_apply_failed`
...
- `maintain-loop`
- `maintain-loop --dry-run`
- `maintain-loop --agent-command <command>`
- `maintain-loop --agent-profile <name>`
...
- `verify-bundle --proposal-path <path> --bundle-path <path> --verifier-command <command>`
- `verify-bundle --proposal-path <path> --bundle-path <path> --verifier-profile <name>`
...
- `run-task --id <id> --verifier-command <command>`
- `run-task --id <id> --verifier-profile <name>`
...
v0.2 object-aware maintenance metadata is additive. `wikify.graph-agent-tasks.v1` remains the task queue schema; tasks may include optional fields such as `target_kind`, `target_family`, `object_id`, `object_type`, `body_path`, `object_path`, `view_path`, `agent_artifact_path`, `source_refs`, `review_status`, and `regeneration_command`. Older readers can ignore those fields.

Artifact-health findings are represented as queued work, not silent content mutation:
- `queue_object_validation_repair` for object validation report records
- `queue_generated_page_repair` for generated page drift or wikiization repair
- `queue_view_regeneration` with `regeneration_command: "wikify views"`
- `queue_agent_export_refresh` with `regeneration_command: "wikify agent export"`

V1/v0.2 安全规则：`maintain` 不修改 topic、parsed、sorted、wiki/pages、views、artifacts/agent 等正文或派生产物，也不在 CLI 内隐藏调用 LLM。它不为了维护上下文重新读取 raw source，也不隐式执行 `sync`、`wikiize`、`views` 或 `agent export`。断链修复、孤立对象挂接、digest refresh、community synthesis、validation repair、view regeneration 和 agent export refresh 都进入 queued plan step 和 agent task queue；只有确定性维护记录可标记为 executed。
...
安全规则：`maintain-run` 只组合 `maintain` 和 `run-tasks`，不新增 apply 语义，不并发执行，不隐藏 provider、模型、密钥或 retry。只有显式 `--agent-command` 或 `--agent-profile` 才会触发外部 command。

### `maintain-loop`
推荐用作低打扰的 bounded maintenance loop。

常用：
- `maintain-loop --dry-run`
- `maintain-loop --max-rounds 3 --task-budget 15 --limit 5 --agent-command "python3 agent.py"`
- `maintain-loop --max-rounds 3 --task-budget 15 --limit 5 --agent-profile default`
- `maintain-loop --max-rounds 3 --task-budget 15 --limit 5 --agent-profile`

执行流程：
- 每轮调用现有 `maintain-run`
- 每轮 limit 受剩余 task budget 限制
- 默认 `max_rounds=3`、`task_budget=15`、`limit=5`
- dry-run 只预览一轮，避免重复 replay 同一个 in-memory queue
- stop_reason 明确写入返回结果

`wikify.maintenance-loop.v1` schema:
...
状态 / stop_reason：
- `no_tasks`
  - 维护刷新成功，筛选后没有可继续执行的 task
- `waiting_for_patch_bundle`
  - 至少一轮进入等待 bundle 状态
- `failed_tasks`
  - 至少一个 task 失败
- `task_budget_exhausted`
  - 已选 task 数达到 `--task-budget`
- `max_rounds_reached`
  - 已达到 `--max-rounds`
- `dry_run_preview`
  - 已完成一轮 dry-run preview

安全规则：`maintain-loop` 是 bounded orchestration。它只重复组合 `maintain-run`，不新增 apply 语义，不并发执行，不隐藏 provider、模型、密钥或 retry。只有显式 `--agent-command` / `--agent-profile` 才会触发外部 producer command；只有显式 `--verifier-command` / `--verifier-profile` 才会触发外部 verifier command。

### `verify-bundle`
推荐用作 apply 前的 agent review gate。

常用：
- `verify-bundle --proposal-path sorted/graph-patch-proposals/agent-task-1.json --bundle-path sorted/graph-patch-bundles/agent-task-1.json --verifier-command "python3 verifier.py"`
- `verify-bundle --proposal-path sorted/graph-patch-proposals/agent-task-1.json --bundle-path sorted/graph-patch-bundles/agent-task-1.json --verifier-profile reviewer`
- `run-task --id agent-task-1 --agent-profile default --verifier-profile reviewer`
- `maintain-loop --agent-profile default --verifier-profile reviewer`

执行流程：
- 读取 proposal 和 patch bundle
- 先运行 deterministic `preflight_patch_bundle`
- preflight includes generated page `source_refs` / `review_status` preservation checks
- 构造 `wikify.patch-bundle-verification-request.v1`
- 将 request JSON 写入 verifier command stdin
- verifier stdout 必须返回 `wikify.patch-bundle-verdict.v1`
- accepted=true 时写 verification artifact 并允许后续 apply
- accepted=false 时写 verification artifact，然后返回 `patch_bundle_verification_rejected`
- 如果 rejection 发生在 `run-task` 自动化路径中，runner 会额外把 task 标记为 `blocked` 并写入 verifier feedback

`wikify.patch-bundle-verification-request.v1` schema:
...
- path: `sorted/graph-patch-verifications/<task-id>.json`
- schema: `wikify.patch-bundle-verification.v1`
- includes request, preflight, verifier command evidence, verdict, status, and artifact path

run-task feedback:
- task field: `blocked_feedback`
- event field: `details`
- required details: `summary`, `findings`, `verification_path`, `verdict`
...
安全规则：verifier gate 不会绕过 deterministic preflight，也不会直接修改正文。generated page preservation failure 会在任何外部 verifier command 执行前返回。它只在 apply 前增加显式外部 agent review。dry-run 不执行 verifier command，也不写 verification artifact。
...
```json
{
  ...
  "safety": {
    ...
    "generated_page_preservation": {
      "required": true,
      "fields": ["source_refs", "review_status"]
    }
  }
}
```

`safety.generated_page_preservation` only appears when the proposal write scope includes generated wiki pages. Producers must preserve generated page `source_refs` and `review_status` exactly; these fields are treated as source-traceability control data, not free-form page content.
...
- `maintain-loop --agent-profile <name>`
- `verify-bundle --verifier-profile <name>`
- `run-task --verifier-profile <name>`
- `run-tasks --verifier-profile <name>`
- `maintain-run --verifier-profile <name>`
- `maintain-loop --verifier-profile <name>`

这些命令也支持裸 `--agent-profile`，表示使用 `default_profile`：
- `maintain-run --agent-profile`
- `maintain-loop --agent-profile`
- `run-tasks --agent-profile`
- `run-task --id agent-task-1 --agent-profile`
- `produce-bundle --request-path <path> --agent-profile`
- `run-task --id agent-task-1 --verifier-profile`
- `verify-bundle --proposal-path <path> --bundle-path <path> --verifier-profile`
...
安全规则：profile 只是显式 command 的别名。`default_profile` 只是裸 `--agent-profile` 或裸 `--verifier-profile` 的名称来源，不会让命令在没有显式 profile flag 时自动执行外部 command。`maintain-loop` 也遵守同一条边界。不要把 API key、token 或私密 provider 配置写进 `wikify-agent-profiles.json`；需要密钥时，应让外部 command 自己从环境变量或安全配置读取。
...
- 对 generated wiki page 执行 preservation preflight，拒绝修改 `source_refs` 或 `review_status`
...
- generated wiki page 的 `source_refs` 或 `review_status` 被改动时返回 `generated_page_preservation_failed`，exit code 为 `2`

安全规则：`apply` 只消费显式 patch bundle，不生成语义内容，不调用隐藏 LLM，不改变 task status。generated wiki page 的 `source_refs` 和 `review_status` 在本地 deterministic preflight 中强制保护，不依赖外部 verifier。task 状态仍由 `tasks` lifecycle 命令显式推进。
...
- verifier 拒绝返回 `patch_bundle_verification_rejected`，exit code 为 `2`，`details.phase` 为 `bundle_verifier`，并包含 `verification_path`、`agent_tasks`、`task_events` 和 `verdict`
