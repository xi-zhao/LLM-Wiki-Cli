...
- bundle 缺失时返回 `waiting_for_patch_bundle`，交给 agent 读取 request 并生成补丁，不打扰用户
...
状态语义：
- `waiting_for_patch_bundle`：proposal 和 bundle request 已准备好，但还没有可应用 bundle；agent 应读取 `artifacts.patch_bundle_request`，再按 request 写 patch bundle
