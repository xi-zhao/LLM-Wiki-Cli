...
## 3.12 Agent Task Workflow Runner
...
- bundle 缺失时返回 `waiting_for_patch_bundle`，交给 agent 生成补丁，不打扰用户
...
状态语义：
- `waiting_for_patch_bundle`：proposal 已准备好，但还没有可应用 bundle；agent 应读取 proposal 并写 patch bundle
