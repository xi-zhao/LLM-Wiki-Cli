...
本文件把当前仓库约定为一个由 LLM 维护的持久化知识库，而不是一次性资料堆。

## 1. 目标

LLM Wiki 的目标不是在用户提问时临时从原文里重新检索一遍，而是把信息逐步编译成可复用、可链接、可持续维护的 markdown wiki。
...
执行规则：
1. 优先调用统一入口：`python3 scripts/ingest_any_url.py "<url>"`
