...
class WikifyCliTests(unittest.TestCase):
    ...
    def test_docs_describe_human_ingest_and_machine_pipeline_boundary(self):
        ...
        self.assertIn('把链接、文件、仓库和笔记变成一个由 Agent 维护的本地 Wiki。', readme)
        self.assertIn('为什么值得 Star', readme)
        self.assertIn('30 秒体验', readme)
        self.assertIn('给 OpenClaw、Codex、Claude Code 和 shell agent 用', readme)
        self.assertIn('人类 Wiki', readme)
        self.assertIn('Agent 记忆', readme)
        self.assertIn('恢复层', readme)
        self.assertIn('不是又一个笔记软件，也不是又一个聊天 RAG demo。', readme)
        self.assertIn('深入文档', readme)
        self.assertIn('人类应该看到什么', readme)
        self.assertIn('wikify ingest <locator>', readme)
        self.assertIn('人类说“帮我保存这篇文章”', readme)
        ...
        self.assertIn('Agent 操作指南', readme)
        self.assertIn('`wikify sync` 现在仍然不会抓取 URL sources', readme)
